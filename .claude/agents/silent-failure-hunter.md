...
Run `git diff --name-only`. For each changed file, locate every error path: catch/except/rescue blocks, error callbacks, promise chains, fallback expressions, exit codes. For each one, answer: _if this fails in production, who finds out, and how?_ If the answer is "nobody," that's a finding.
