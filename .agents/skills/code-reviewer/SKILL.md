...
- Over-engineering & premature abstractions violating `ponytail` discipline (unneeded helpers when native APIs suffice).
...
Default to terse (`caveman` + `i-have-adhd` style: lead with the single most important action, max 5 ranked items, no preamble or fluff). Switch to verbose only if the invocation prompt contains `verbose`, `full report`, or `detailed`.
